...
import { X, RefreshCw, CheckCircle2, AlertCircle, Laptop } from "lucide-react";
...
export function ToggleStatusModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fadeIn">
      <div className="relative w-full max-w-md bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-7 shadow-2xl">
        <button
          onClick={onClose}
        ...
        <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 mb-4 space-y-2 text-xs">
          ...
          <div className="flex justify-between text-slate-400">
            <span>Guruhi:</span>
            <span className="text-slate-200">{student.group}</span>
          </div>
          <div className="flex justify-between text-slate-400">
            <span>Holat o'zgarishi:</span>
            <span className={isCurrentlyTaken ? "text-emerald-400 font-bold" : "text-red-400 font-bold"}>
        ...
        <div className="flex gap-3">
          ...
          <button
            ...
            className={`w-2/3 py-2.5 text-white rounded-xl text-sm font-bold shadow-lg transition-all flex items-center justify-center gap-2 ${
              isCurrentlyTaken
